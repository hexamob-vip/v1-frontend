type DeviceSizeIndicatorProps = {
  enable?: boolean;
};

export default function DeviceSizeIndicator({
  enable = true,
}: DeviceSizeIndicatorProps) {
  if (process.env.NODE_ENV !== "development" || !enable) {
    return null;
  }

  return (
    <div className="pointer-events-none select-none fixed top-0 right-0 z-[999999] m-2 flex h-6 w-6 items-center justify-center rounded-full bg-gray-700 p-3 font-mono text-xs text-white sm:bg-pink-500 md:bg-orange-500 lg:bg-green-500 xl:bg-blue-500 2xl:bg-blue-700">
